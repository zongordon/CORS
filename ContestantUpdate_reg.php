<?php
//Removed "$insert_" from the variables during validation of the form and corrected hidden field to contestant_id in form
ob_start();
...
if (filter_input(INPUT_POST,"MM_update_contestant") === "update_contestant") {
  $contestant_name = encodeToUtf8(mb_convert_case(filter_input(INPUT_POST,'contestant_name'), MB_CASE_TITLE,"UTF-8"));    
  $contestant_birth = filter_input(INPUT_POST,'contestant_birth');
  ...
  $output_form = 'no';
    ...
    if (empty($contestant_name)) {
      // $contestant_name is blank
      echo '<h3>Du gl&ouml;mde att fylla i namn!</h1>';
      $output_form = 'yes';
    }
    if (empty($contestant_birth)) {
      // $contestant_birth is blank
      echo '<h3>Du gl&ouml;mde att fylla i f&ouml;delsedatum!</h1>';
      $output_form = 'yes';
	}
    if (!empty($contestant_birth) && !preg_match('/^\d{4}-\d{2}-\d{2}$/', $contestant_birth)) {
    // $contestant_birth is wrong format
    echo '<h3>Du anv&auml;nde fel format p&aring; f&ouml;delsedatum (YYYY-MM-DD)!</h1>';
    $output_form = 'yes';
    }	
    if (empty($contestant_gender)) {	
      // $contestant_gender is blank
...
<?php 
    ...
    $stmt->bindValue(':contestant_name', $contestant_name, PDO::PARAM_STR);
    $stmt->bindValue(':contestant_birth', $contestant_birth, PDO::PARAM_STR);
    $stmt->bindValue(':contestant_gender', $contestant_gender, PDO::PARAM_STR);
